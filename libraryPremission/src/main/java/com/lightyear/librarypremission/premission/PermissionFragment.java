...
public final class PermissionFragment extends Fragment implements Runnable {
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (grantedPermission.size() == permissions.length) {
            // 代表申请的所有的权限都授予了
            LEasyPermissions.getPermissionInterceptor().grantedPermissions(activity, callBack, grantedPermission, true);
            return;
        }
        ...
        LEasyPermissions.getPermissionInterceptor().deniedPermissions(activity, callBack, deniedPermission, PermissionUtils.isPermissionPermanentDenied(activity, deniedPermission));

        // 证明还有一部分权限被成功授予，回调成功接口
        if (!grantedPermission.isEmpty()) {
            LEasyPermissions.getPermissionInterceptor().grantedPermissions(activity, callBack, grantedPermission, false);
        }
    }
